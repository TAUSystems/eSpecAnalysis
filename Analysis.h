...
void calMode(spectrometer& eSpec, std::string& pathCalibration, std::vector<cv::Mat>& H, std::vector<std::vector<double>>& xRuler, std::vector<std::vector<double>>& yRuler) {
	...
	while (loop) {
		...
		listFile = fileList(listPath);
		std::string refFile = findRef(listFile);
...
void pointingMode(double& rate, double& timeout, spectrometer& eSpec, screenCalibration& calibration, paramSpace & pSpace, std::vector<cv::Mat>& H, std::vector<std::vector<double>>& xRuler, std::vector<std::vector<double>>& yRuler) {
	...
	int pathLength = (int)listPath.length();
	std::string pointingPath = eSpec.screenPath(pI);
	std::string BPath = eSpec.screenPath(sBI);
	std::vector<std::string> listOld = fileList(listPath);
	std::vector<std::string> listNew, listIm;
	std::vector<double> viewResScreen = calibration.viewResolution(screen);
	std::vector<double> viewResPoint = calibration.viewResolution(pI);
	std::vector<double> viewResB = calibration.viewResolution(sBI);
	...
	int Nold, Nnew, Nim;
	int pointIndex = 0;
	imageBW image, imTS, imagePoint, imTP, imTB;
	std::vector<double> line;

	std::vector<double> pxX, pxY;
	int N = (int)std::max(viewResPoint[0], viewResPoint[1]);
	for (int i = 0; i < N; i++) {
		if (i < viewResPoint[0]) {
	...
	pointX = xRuler[0];
	pointY = yRuler[0];
	mRadAxis(eSpec, pI, pointX, pointY);
	double pointing, eval, buffer;
	std::vector<int> acceptanceBound;
	acceptanceBound.resize(4, 0);
	eval = -1.0 * eSpec.angleMax(0);
	FE1DInterp(pointX, pxX, eval, buffer);
	acceptanceBound[0] = (int)round(buffer);
	eval = eSpec.angleMax(0);
	FE1DInterp(pointX, pxX, eval, buffer);
	acceptanceBound[1] = (int)round(buffer);
	eval = -1.0 * eSpec.angleMax(1);
	FE1DInterp(pointY, pxY, eval, buffer);
	acceptanceBound[2] = (int)round(buffer);
	eval = eSpec.angleMax(1);
	FE1DInterp(pointY, pxY, eval, buffer);
	acceptanceBound[3] = (int)round(buffer);
	...
	while (loop) {
		...
		if (Nnew != Nold) {
			...
			for (int i = 0; i < Nnew; i++) {
				if (newItem[i]) {
					fileName = listNew[i].substr(pathLength + 1, listNew[i].length() - pathLength - 1);
					...
					while (fileFindLoop) {
						std::this_thread::sleep_for(std::chrono::milliseconds((long)loopWait));
						fileFindLoop = readImage(listNew[i], image);
						if (fileLoopCount > 30) {
							fileFindLoop = 0;
							fileFound = 0;
							std::cout << "Can not find eScreen A file.\n";
						}
						fileLoopCount = fileLoopCount + 1;
					}
					if (fileFound) {
						perspectiveTransform(image, H[screen], viewResScreen, imTS);
						removeOutlier(imTS, 4.0);
						medianFilter(imTS, 2);
						...
						while (fileFindLoop) {
							std::this_thread::sleep_for(std::chrono::milliseconds((long)loopWait));
							listIm = fileList(pointingPath);
							Nim = (int)listIm.size();
							for (int j = 0; j < Nim; j++) {
								if (listIm[j].find(fileName) != std::string::npos) {
									pointIndex = j;
									fileFindLoop = readImage(listIm[pointIndex], image);
									fileLoopCount = 0;
									break;
								}
							}
							if (fileLoopCount > 30) {
								fileFindLoop = 0;
								readImage(listIm.back(), image);
								flagP = 1;
								std::cout << "Can not find associated pointing file.\n";
							}
							fileLoopCount = fileLoopCount + 1;
						}


						perspectiveTransform(image, H[pI], viewResPoint, imTP);
						removeOutlier(imTP, 4.0);
						medianFilter(imTP, 2);

						double peakValue, peakValueB;
						imTP.crop(acceptanceBound, image);
						findSignalPeak(image, peakBound, peakValueB);
						findSignalPeak(imTP, peak, peakValue);
						peakBound[0] = peakBound[0] + acceptanceBound[0];
						peakBound[1] = peakBound[1] + acceptanceBound[2];
						eval = (double)((double)imTP.sizeY() - 1 - peakBound[1]);
						FE1DInterp(pxY, pointY, eval, pointing);
						//std::cout << std::setprecision(2) << "Pointing:" << pointing << " mrad\n";

						fileFindLoop = 1;
						fileLoopCount = 0;
						while (fileFindLoop) {
							std::this_thread::sleep_for(std::chrono::milliseconds((long)loopWait));
							listIm = fileList(BPath);
							Nim = (int)listIm.size();
							for (int j = 0; j < Nim; j++) {
								if (listIm[j].find(fileName) != std::string::npos) {
									pointIndex = j;
							...
							if (fileLoopCount > 30) {
								...
								readImage(listIm.back(), image);
								flagB = 1;
								std::cout << "Can not find associated eScreen B file.\n";
							}
							fileLoopCount = fileLoopCount + 1;
						}

						
						perspectiveTransform(image, H[sBI], viewResB, imTB);
						removeOutlier(imTB, 4.0);
						medianFilter(imTB, 2);
						...
						resH = 1900;
						double spX, spY;
						spY = ((double)imTS.sizeY() + (double)imTB.sizeY());
						spX = (std::max((double)imTS.sizeX(), (double)imTB.sizeX()));
						ratio = spY / (spX + spY);
						ratio = ratio * resH - 7;
						...
						drawLineX[0] = 0;
						drawLineX[1] = (int)imTP.sizeX() - 1;
						drawLineY[0] = (int)imTP.sizeY() - 1 - peak[1];
						...
						drawLineY[0] = 0;
						drawLineY[1] = (int)imTP.sizeY() - 1;
						plt::plot(drawLineX, drawLineY, { {"color","k"} });

						drawLineX[0] = 0;
						drawLineX[1] = (int)imTP.sizeX() - 1;
						drawLineY[0] = (int)imTP.sizeY() - 1 - peakBound[1];
						drawLineY[1] = drawLineY[0];
						...
						pValue = pValue.substr(0, 4);
						plt::text((int)(0.775 * imTP.sizeX()), (int)(0.975 * imTP.sizeY()), pValue + std::string(" mrad"));
						if (flagP) {
							if (flagB) {
								plt::text((int)(0.800 * imTP.sizeX()), (int)(0.925 * imTP.sizeY()), std::string("Error P,B"));
							}
							else {
								plt::text((int)(0.800 * imTP.sizeX()), (int)(0.925 * imTP.sizeY()), std::string("Error P"));
							}
						}
						else {
							if (flagB) {
								plt::text((int)(0.800 * imTP.sizeX()), (int)(0.925 * imTP.sizeY()), std::string("Error B"));
							}
						}

						drawAxis(1, eSpec, pSpace, pI, xRuler[pI], yRuler[pI], pointing);

						plt::axis("off");
						plt::subplot2grid(2, (int)((spY + spX) / spY), 0, 1, 1, (int)(spX / spY));
						imshow(imTS, 1, "");
						drawAxis(1, eSpec, pSpace, screen, xRuler[screen], yRuler[screen], pointing);
						plt::axis("off");
						plt::subplot2grid(2, (int)((spY + spX) / spY), 1, 1, 1, (int)(spX / spY));
						imshow(imTB, 1, "");
						drawAxis(1, eSpec, pSpace, sBI, xRuler[sBI], yRuler[sBI], pointing);
						plt::axis("off");
